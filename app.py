...
def save_config(user_id, **kwargs):
    try:
...
def save_note(user_id, title, content, tags=None):
    try:
...
def send_email_action(to, subject, body):
    ...
    if not se or not sp:
        return False, "Email not configured. Go to Settings to add Gmail + App Password."
    try:
...
def get_weather_action(city):
    ...
    try:
        ...
        return False, f"Weather failed: {e}"
...
def try_local_command(text):
    ...
    if calc_match:
        ok, msg = calc_action(calc_match.group(1).strip())
        return True, msg, None

    # Direct math expression (e.g. "2+2", "sqrt(144)")
    if re.match(r'^[\d\s\+\-\*\/\.\(\)sqrtpi]+$', t) and any(c in t for c in '+-*/'):
        ok, msg = calc_action(t)
        return True, msg, None

    # Weather
    weather_match = re.match(r'(?:weather|temperature|temp)\s*(?:in|for|at|of)?\s*(.+)', t)
    if weather_match:
        ok, msg = get_weather_action(weather_match.group(1).strip())
        return True, msg, None
    ...
    if note_match:
        ...
        if ok:
            return True, f"Note saved: **{title}**", None
    ...
    # Delete note
    del_note_match = re.match(r'(?:delete|remove)\s*(?:the\s*)?(?:last\s*)?note', t)
    if del_note_match:
        notes = get_notes(st.session_state.user["id"])
        if notes:
            delete_note(notes[0]["id"])
            return True, f"Deleted note: **{notes[0]['title']}**", None
    ...
    if email_match:
        ...
        body = email_match.group(2).strip() if email_match.group(2) else "Please see this message."
        subject = "Message from ANU"
        ok, msg = send_email_action(to, subject, body)
        return True, msg, None
    ...
    if url_match or any(w in t for w in ["open ", "go to ", "visit ", "navigate to "]):
        if url_match:
            ...
            url = t.replace("open", "").replace("go to", "").replace("visit", "").replace("navigate to", "").strip()
            if not url.startswith("http"):
                url = "https://" + url
        return True, f"Opening [{url}]({url})...", url

    # Open app (Windows)
    app_match = re.match(r'(?:open|launch|start)\s+(?:app|application|program)?\s*(.+)', t)
    if app_match and os.name == 'nt':
        app = app_match.group(1).strip()
        app_map = {"chrome": "chrome", "firefox": "firefox", "edge": "msedge", "notepad": "notepad", "calculator": "calc", "settings": "control", "explorer": "explorer", "terminal": "cmd", "cmd": "cmd", "word": "winword", "excel": "excel"}
        exe = app_map.get(app, app)
        try:
            import subprocess
            subprocess.Popen(exe, shell=True)
            return True, f"Opening {app}...", None
        except Exception as e:
            return True, f"Couldn't open {app}: {e}", None
    ...
    # Greeting / small talk (respond locally, no API)
    ...
    if any(t == g or t.startswith(g + " ") for g in greetings):
        name = st.session_state.user.get("full_name", "there")
        return True, f"Hey {name}! 👋 What can I do for you?", None

    thanks = ["thanks", "thank you", "thx", "ty"]
    if any(t == th for th in thanks):
        return True, "You're welcome! 😊", None

    who_are = ["who are you", "what are you", "what is anu", "tell me about yourself"]
    if any(t == w for w in who_are):
        return True, "I'm **ANU** — your Virtual Desktop Assistant. I can send emails, search the web, check weather, open apps, calculate, save notes, and more. Just tell me what to do! 🤖", None

    return False, None, None

# ─── Gemini (only for complex tasks) ──────────────────────────────────────────
def get_gemini(prompt, history):
    ...
    model = genai.GenerativeModel(
        ...
        system_instruction=(
            f"You are ANU, a virtual desktop assistant for {st.session_state.user.get('full_name', 'the user')}. "
            "Be concise, helpful, and action-oriented. Keep responses short."
        ),
    )
...
def page_onboarding():
    ...
    <div style="text-align:center;margin:30px 0 10px;">
        ...
        <p style="color:#8892b0;">Quick setup — 1 minute.</p>
    </div>
    ...
    with col:
        if step == 1:
            ...
            st.markdown('<div style="text-align:center;padding:40px 20px;"><div style="font-size:4rem;margin-bottom:16px;">🎉</div><h2 style="color:#e6f1ff;">ANU is ready!</h2><p style="color:#8892b0;">Just tell ANU what to do — chat or voice.</p></div>', unsafe_allow_html=True)
...
def page_app():
    ...
    with st.sidebar:
        ...
        # Fixed navigation with explicit rerun
        tab = st.session_state.active_tab
        chat_active = "💬" if tab == "Chat" else "○"
        voice_active = "🎤" if tab == "Voice" else "○"
        settings_active = "⚙️" if tab == "Settings" else "○"

        if st.button(f"{chat_active}  Chat", key="nav_chat", use_container_width=True, type="primary" if tab == "Chat" else "secondary"):
            st.session_state.active_tab = "Chat"
            st.rerun()
        if st.button(f"{voice_active}  Voice", key="nav_voice", use_container_width=True, type="primary" if tab == "Voice" else "secondary"):
            st.session_state.active_tab = "Voice"
            st.rerun()
        if st.button(f"{settings_active}  Settings", key="nav_settings", use_container_width=True, type="primary" if tab == "Settings" else "secondary"):
    ...
    if tab == "Chat":
        if not st.session_state.messages:
            st.markdown(f'<div class="chat-anu"><b>ANU 🤖</b><br>Hey {full_name}! I\'m ANU, your virtual desktop assistant. Tell me what to do:<br><br>• "Send email to john@gmail.com about the meeting"<br>• "Weather in Hyderabad"<br>• "Open github.com"<br>• "Search quantum computing"<br>• "Calculate 25 * 48"<br>• "Save a note about the project"<br>• "Check system status"<br>• "Tell me a joke"<br><br>For complex tasks like code generation or creative writing, I\'ll use AI. Simple tasks cost nothing! 😊</div>', unsafe_allow_html=True)
        ...
        if user_input:
            ...
            # Try local command first (free)
            handled, response, action_url = try_local_command(user_input)

            if handled:
                st.session_state.messages.append({"role": "assistant", "content": response})
                if action_url:
                    st.session_state.messages.append({"role": "action", "content": f"🔗 [Open Link]({action_url})"})
            else:
                # Fall back to Gemini (costs API quota)
                with st.spinner("ANU is thinking..."):
                    response = get_gemini(user_input, st.session_state.messages[:-1])
                st.session_state.messages.append({"role": "assistant", "content": response})
            ...
            if st.session_state.voice_text:
                if st.button("📨 Execute Command", use_container_width=True):
                    ...
                    if handled:
                        ...
                        with st.spinner("ANU is thinking..."):
                            response = get_gemini(text, st.session_state.messages[:-1])
                        st.session_state.messages.append({"role": "assistant", "content": response})
    ...
    # ── SETTINGS ──────────────────────────────────────────────────────────────
        ...
        with st.expander("🌤️ Weather API"):
            ...
            if st.button("Save", key="save_weather"):
        ...
        with st.expander("📝 My Notes"):
            ...
            if not st.session_state.notes:
                st.info("No notes yet. Tell ANU 'save a note about...' in chat!")
            else:
